...
import { formatFullName, getResumeUrl } from "@/lib/utils";
...
type ExperienceData =
  | JobPosition[]
  | { positions_held?: JobPosition[]; years?: string; industries?: string[] }
  | string;
...
const CandidateProfile = () => {
  ...
  const [transformedResumeUrl, setTransformedResumeUrl] = useState<
    string | null
  >(null);
      ...
      try {
        ...
        if (profileResult.data) {
          ...
          if (!profileResult.error.message?.includes("No rows found")) {
            ...
            if (
              profileResult.error.message?.includes(
                'relation "public.candidate_profiles" does not exist'
              )
            ) {
    ...
    if (candidate && candidate.resume_analysis) {
      try {
        ...
        const parsedAnalysis =
          typeof candidate.resume_analysis === "string"
  ...
  // Transform resume URL for local development
  useEffect(() => {
    const transformUrl = async () => {
      if (candidate?.resume_url) {
        const transformedUrl = await getResumeUrl(candidate.resume_url);
        setTransformedResumeUrl(transformedUrl);
      }
    };
  ...
  const getPositions = (): JobPosition[] => {
    ...
    if (enrichedProfile?.experience) {
      const expData = parseJsonSafely<
        JobPosition[] | { positions_held?: JobPosition[] }
      >(
      ...
      if (expData) {
        ...
        if (expData.positions_held && Array.isArray(expData.positions_held))
          return expData.positions_held;
      }
    }
    ...
    if (
      ...
      return candidate.resume_analysis.experience
        .positions_held as JobPosition[];
    }
  ...
  const location =
    ...
    (candidate.resume_analysis?.personal_info?.geographic_location as
      | string
      | undefined) ||
    "";
  ...
  const email =
    candidate.email ||
    (candidate.resume_analysis?.personal_info?.email as string | undefined) ||
    "";
  const phone =
    candidate.phone ||
    enrichedProfile?.phone ||
    (candidate.resume_analysis?.personal_info?.phone as string | undefined) ||
    "";
  ...
  const jobTitle =
    ...
    candidate.resume_analysis.experience.positions_held.length > 0
      ? (candidate.resume_analysis.experience.positions_held[0] as JobPosition)
          .title
      : "");
  ...
  const resumeSkills = Array.isArray(candidate.resume_analysis?.skills)
    ...
    : [];
  const enrichedSkills = Array.isArray(enrichedProfile?.skills)
    ? enrichedProfile.skills
    : [];

  // Combine skills from all sources and remove duplicates
  const skills = [
    ...new Set([...candidateSkills, ...resumeSkills, ...enrichedSkills]),
  ];
  ...
  const getEducation = (): Education[] => {
    ...
    if (enrichedProfile?.education) {
      ...
      if (Array.isArray(enrichedProfile.education))
        return enrichedProfile.education;
    }
      ...
      <div className="flex flex-col md:flex-row gap-6 items-start">
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            {transformedResumeUrl && (
              <a
                href={transformedResumeUrl}
                target="_blank"
                rel="noopener noreferrer"
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="flex flex-wrap gap-2">
                      {(
                        candidate.resume_analysis
                          .areas_specialization as string[]
                ...
                <Card className="md:col-span-2">
                  ...
                  <CardContent>
                    <ul className="list-disc list-inside space-y-2 pl-2">
                      {(
                        candidate.resume_analysis
                          .notable_achievements as string[]
                  ...
                  <div className="space-y-6">
                    {experience.map((job: JobPosition, index: number) => (
                      <div key={index} className="border-b pb-4 last:border-0">
                        ...
                        {formatResponsibilities(job.responsibilities).length >
                          0 && (
                          <div className="mt-2">
                            <ul className="list-disc list-inside text-sm space-y-1 pl-2">
                              {formatResponsibilities(job.responsibilities).map(
                                (item, idx) => (
                                  <li key={idx}>{item}</li>
                                )
                              )}
              ...
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  {candidate?.resume_analysis?.education &&
                    Array.isArray(candidate.resume_analysis.education) &&
                    candidate.resume_analysis.education.some(
                      (edu: any) => edu.field || edu.graduation_year
                    ) && (
                      <Badge
                        variant="outline"
                        className="ml-2 text-xs border-blue-500 text-blue-600"
                      >
                        Verified
                      </Badge>
                    )}
                    ...
                    {education.map((edu: Education, index: number) => {
                      ...
                      const eduData = (() => {
                        ...
                        if (
                          ...
                          typeof candidate.resume_analysis.education[index] ===
                            "object"
                        ) {
                          const resumeEdu = candidate.resume_analysis.education[
                            index
                          ] as any;
                          return {
                            ...edu,
                            ...
                            graduation_year:
                              resumeEdu.graduation_year || edu.graduation_year,
                            gpa: resumeEdu.gpa || edu.gpa,
                            description:
                              resumeEdu.description || edu.description,
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="mt-1 space-y-1">
                            ...
                            {/* Activities */}
                            {eduData.activities &&
                              eduData.activities.length > 0 && (
                                <div className="mt-2">
                                  <p className="text-sm font-medium">
                                    Activities:
                                  </p>
                                  <ul className="list-disc list-inside text-sm space-y-1 pl-2">
                                    {eduData.activities.map(
            ...
            {hasEnrichedData && (
              <Card className="md:col-span-2">
                <CardHeader>
                  ...
                  <CardDescription>
                    Additional data from People Data Labs
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid gap-6 md:grid-cols-2">
