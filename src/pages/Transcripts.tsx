  ...
  useEffect(() => {
    ...
    const fetchSessions = async () => {
      try {
        ...
        // If a company is selected, first get all positions for that company
        if (selectedCompany) {
          console.log("Filtering by company_id:", selectedCompany.id);
          ...
          positionIds = positionsData?.map((p) => p.id) || [];
          console.log("Found position IDs for company:", positionIds);

          // If no positions found for this company, return early with empty results
        ...
        // Apply position filter if we have specific positions
        if (positionIds.length > 0) {
        ...
        if (sessionsData && sessionsData.length > 0) {
          console.log("Sample session company_id:", sessionsData[0].company_id);
        }
        console.log(
          "Supabase URL:",
          (window as any).supabase?._supabaseUrl ||
            (window as any).__supabaseClient?._supabaseUrl
        );
        ...
        const sessionsWithTranscripts = sessionsData.map((session) => ({
          ...
          transcript_entries:
            transcriptsData?.filter((t) => t.session_id === session.id) || [],
        }));
            ...
            <Card className="h-full flex flex-col">
              <CardHeader>
                <CardTitle>
                  {selectedSession
                    ? `${formatFullName(
                        selectedSession.candidate.first_name,
                        selectedSession.candidate.last_name
                      )} - ${selectedSession.position.title}`
                    : "Transcript Details"}
                </CardTitle>
                <CardDescription>
                  {selectedSession
              ...
              <CardContent className="flex-1">
                {selectedSession ? (
                  ...
                  <div className="flex flex-col items-center justify-center min-h-[400px] text-center text-muted-foreground">
                    ...
                    <p className="text-sm">
                      Transcripts contain detailed conversation records and
                      analysis
                    </p>
                  </div>
                )}
