...
const SessionList: React.FC<SessionListProps> = ({ filterStatus }) => {
  ...
  useEffect(() => {
    fetchSessions();
  }, [currentPage, statusFilter, selectedCompany]);
    ...
    try {
      ...
      let query = supabase
        ...
        .select(
          ...
          position:positions!interview_sessions_position_id_fkey (id, title, company_id),
          start_time,
          status,
  ...
  const getInvitationStatus = (session: InterviewSession) => {
    if (
      !session.interview_invitations ||
      session.interview_invitations.length === 0
    ) {
          ...
          <div>
            ...
            <CardDescription>
              ...
              {statusFilter === "completed" &&
                "Review past interview sessions and transcripts"}
              {statusFilter === "all" &&
                        ...
                        <div className="flex flex-col items-center justify-center p-4">
                          ...
                          <p className="text-sm text-muted-foreground">
                            {searchTerm
                              ...
                              : statusFilter === "scheduled"
                              ? "Schedule an interview session to get started"
                              : statusFilter === "in_progress"
                              ? "Start a scheduled interview to see it here"
                              : statusFilter === "completed"
